...
namespace Movies.Services
{
    public class MovieApiService : IMovieApiService
    {
        ...
        public async Task<MovieApiResponse> SearchByTitleAsync(string title , int page = 1)
        {
            ...
            string key = title + "_" + page;
            MovieApiResponse result;
          //  if (!memoryCache.TryGetValue(key, out result))
            {
                var response = await httpClient.GetAsync($"{movieApiOptions.BaseUrl}?s={title}&apikey={movieApiOptions.ApiKey}&page=${page}");
                var json = await response.Content.ReadAsStringAsync();
           //     result = JsonConvert.DeserializeObject<MovieApiResponse>(json); // JsonSerializer.Deserialize<MovieApiResponse>(json);
                result = System.Text.Json.JsonSerializer.Deserialize<MovieApiResponse>(json);

                if (result.Response == "False")
                    throw new Exception(result.Error);

                var cacheExpire = new MemoryCacheEntryOptions().SetSlidingExpiration(TimeSpan.FromDays(1));
          //      memoryCache.Set(key, result, cacheExpire);
          ...
          //  if (!memoryCache.TryGetValue(id,out result))
            {
                ...
                var json = await response.Content.ReadAsStringAsync();
            //    result = JsonConvert.DeserializeObject<Movie>(json); //JsonSerializer.Deserialize<Movie>(json);
                result = System.Text.Json.JsonSerializer.Deserialize<Movie>(json);
