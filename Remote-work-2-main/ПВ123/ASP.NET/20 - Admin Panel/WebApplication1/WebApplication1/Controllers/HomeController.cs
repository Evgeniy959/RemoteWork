...
namespace WebApplication1.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            IQueryable<Post> posts = blogDbContext.Posts
                .Include(x => x.PostTags).ThenInclude(x=>x.Tag)
                .Include(x => x.Category)
                .OrderByDescending(x=>x.Id);//.ThenInclude(x => x.Tag).Include(x => x.Category);


            if (search != null)
            {
                posts = posts.Where(x => x.Title.Contains(search));
            }
            ...
            if (tagId != null)
            {
                //var postIdsByTags = blogDbContext.PostTags.Where(x => x.TagId == tagId).Select(x=>x.PostId);
                //posts = posts.Where(x => postIdsByTags.Contains(x.Id));

                posts = posts.Where(x => x.PostTags.ToList().Any(x => x.TagId == tagId));
            }


            double postSize = 3.0;
            var totalPages = (int)Math.Ceiling(posts.Count() / postSize);

            posts = posts.Skip((int)(postSize * (page - 1))).Take((int)postSize);

            var homeindexViewModel = new HomeIndexViewModel
            {
                Posts = posts,//blogDbContext.Posts.Include(x => x.PostTags).ThenInclude(x => x.Tag).Include(x => x.Category),
                ...
                CountPosts = blogDbContext.Posts.Count() ,
                RouteParam = new Dictionary<string, string> { { "categoryId", categoryId.ToString() }, {"tagId",tagId.ToString() }, { "page", page.ToString() }, { "search", search } },
                TagId = tagId,
        ...
        //public IActionResult Index(int? categoryId, int? tagId,string? search)
        //{
